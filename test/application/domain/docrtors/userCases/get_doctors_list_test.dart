...
import 'package:consultations_app_test/application/domain/doctors/useCases/get_doctors_list.dart';
...
main() {
  ...
  late GetDoctorsList useCase;

  setUp(() {
    mockDoctorsRepository = MockDoctorsRepository();
    useCase = GetDoctorsList(mockDoctorsRepository);
  });

  test('Should call getDoctorList from repository when executed', () async {
    await useCase.execute(
      const EmptyRequest(),
    );
    verify(mockDoctorsRepository.getDoctorsList);
  });
  ...
  test(
      ...
      () async {
    when(
      () => mockDoctorsRepository.getDoctorsList(),
    ).thenAnswer(
